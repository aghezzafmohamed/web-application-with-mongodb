    ...
    <section id="container" >
      ...
      <section id="main-content">
        <section class="wrapper">
          <h3><i class="fa fa-desktop"></i> Ouvrages de recherche avec ISBN</h3>
          <hr/>
          ...
          $i=0;
          if(!empty($_POST["auteurs"]) && empty($_POST["Titre_complet"]) && empty($_POST["Maison_Edition"]) && empty($_POST["Lieu_Edition"]) && empty($_POST["Annee"]) && empty($_POST["ISBN/ISSN"]) && empty($_POST["Depot_legal"])){
          $re=$collectionProjet->dropIndexes();
          $ee=$collectionProjet->createIndex(["auteurs"=>"text"]);
          if(isset($choix) && isset($sort)){
          $docs=$collectionProjet->find(['$text'=>['$search'=>$_POST['auteurs']],'type'=>"Ouvrages de recherche avec ISBN"],['limit'=>$limit,'sort' => [$choix =>$sort]]);
          ...
          $docs=$collectionProjet->find(['$text'=>['$search'=>$_POST['auteurs']],'type'=>"Ouvrages de recherche avec ISBN"],['limit'=>$limit]);
          foreach($docs as $valk){
          $i=$i+1;
          }
          echo "<h5><p align='left' color='blue'><font color='orange'><b>&nbsp&nbsp &nbsp &nbsp ".$i."</b>&nbsp Element(s)</font></p></h5>" ;
