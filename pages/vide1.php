							...
							<form method="post">
								...
								<table id="todo" class="table table">
									<tbody>
										...
										collectionProjet </td><td><input type="radio" name="valuecollection" value="collectionProjet" checked>
									</span></td></tr>
									<tr><td> <span  class="check">
										authentification </td><td><input type="radio" name="valuecollection" value="authentification">
									</span></td></tr>
								</tbody>
							</table>
							<center>
							<input  type="submit" onclick="return confirm('Voulez-vous vraiment supprimer cette collection ?')" name="envoic" value="Supprimer" class="btn btn-theme04"/></center>
						</form>
					</div>
				</div>
				<?php
				if(isset($_POST['envoic'])){
				if(!empty($_POST['valuecollection'])){
				
				require "outils/vendor/autoload.php";
				$client=new MongoDB\Client;
				$dbProjet=$client->dbProjet;
				
				if($_POST['valuecollection']=="collectionProjet"){
				$collectionProjet=$dbProjet->collectionProjet;
				$collection=$collectionProjet->drop();
				echo'<script>
				window.location.replace("index.php"); </script>';
				}
				else if ($_POST['valuecollection']=="authentification"){
				$authentification=$dbProjet->authentification;
				$collection=$authentification->drop();
				echo'<script>
				window.location.replace("index.php"); </script>';
				}
				}}
