      ...
      <header class="header black-bg">
        ...
        <section id="main-content">
          <section class="wrapper">
            <h3><i class="fa fa-desktop"></i> Chapitres d’ouvrage indexés dans des bases de données internationales</h3>
            <hr/>
            ...
            $i=0;
            if(!empty($_POST["auteurs"]) && empty($_POST["Titre_complet"]) && empty($_POST["Titre_de_louvrage"]) && empty($_POST["Maison_Edition"]) && empty($_POST["Annee"])  && empty($_POST["LienURL"])){
            $re=$collectionProjet->dropIndexes();
            $ee=$collectionProjet->createIndex(["auteurs"=>"text"]);
            if(isset($choix) && isset($sort)){
            $docs=$collectionProjet->find(['$text'=>['$search'=>$_POST['auteurs']],'type'=>"Chapitres d’ouvrage indexés dans des bases de données internationales"],['limit'=>$limit,'sort' => [$choix =>$sort]]);
            ...
            echo "<div class='col-lg-12' >
              <div class='form-panel'>
                ...
                echo "<tr><td><font size='3.5px'><b>Lien URL </b></font></td><td style='word-spacing: 2px;letter-spacing: 1px;'><b><a target='_blank' href=".$valk['LienURL'].">Accéder au lien</a></b></td></tr>";
              echo '</table><br>';
            echo "   </div></div><br> <br>";
            }
            }else {
            $docs=$collectionProjet->find(['$text'=>['$search'=>$_POST['auteurs']],'type'=>"Chapitres d’ouvrage indexés dans des bases de données internationales"],['limit'=>$limit]);
            foreach($docs as $valk){
            $i=$i+1;
            }
            echo "<h5><p align='left' color='blue'><font color='orange'><b>&nbsp&nbsp &nbsp &nbsp ".$i."</b>&nbsp Element(s)</font></p></h5>" ;
